    ...
    parseGMXFile(gmxFilePath) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return [].concat(...projectParts.map(projectPart => Array.from(xmlDoc.querySelectorAll(`assets ${projectPart}s ${projectPart}`))
                .map(x => ({
                text: path.parse(x.textContent).name.replace(`${projectPart}s\\`, ''),
                type: "variable",
                ...
                .map(x => ({
                text: path.parse(x.textContent).name.replace("scripts\\", ""),
                type: "function",
                rightLabel: "script"
            }))).concat(Array.from(xmlDoc.querySelectorAll("assets objects object"))
                .map(x => ({
                text: path.parse(x.textContent).name.replace("objects\\", ""),
                type: "class",
                rightLabel: "class"
            })));
        });
